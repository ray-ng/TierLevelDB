...
Options SanitizeOptions(const std::string& dbname,
  ...
  if (result.vlog_block_cache == nullptr) {
    result.vlog_block_cache = NewLRUCache(8 << 20);
  }
  return result;
}
  ...
  for (std::string& filename : filenames) {
    if (ParseFileName(filename, &number, &type)) {
      ...
      if (!keep) {
        ...
        if (type == kTableFile || type == kVLogFile) {
...
Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
  ...
  smeta.number = versions_->NewFileNumber();
  meta.number = versions_->NewFileNumber();
  vmeta.number = versions_->NewFileNumber();
  ...
  {
    mutex_.Unlock();
    s = BuildTable(dbname_, env_, options_, table_cache_, iter, &meta, &vmeta);
    mutex_.Lock();
  }
  ...
  pending_outputs_.erase(vmeta.number);
  ...
  if (s.ok()) {
    smeta.smallest = meta.smallest;
    smeta.largest = meta.largest;
    const Slice min_user_key = smeta.smallest.user_key();
    const Slice max_user_key = smeta.largest.user_key();
    if (base != nullptr) {
...
Status DBImpl::DoCompactionWorkWrapper(CompactionState* compact, 
  ...
  if (!is_manual && c->IsTrivialMoveFile(input_idx)) {
    ...
    assert(c->num_input_files_idx(0, input_idx) == 1);
    ...
    status = DoCompactionWork(compact, input_idx);
  }
  return status;
}
...
void DBImpl::BackgroundCompaction() {
  ...
  if (c == nullptr) {
    ...
    SetMetaData smeta;
    FileMetaData meta, vmeta;
    SetMetaData* s = c->set_at_idx(0, 0);
    for (size_t i = 0; i < s->physical_files.size(); i++) {
      FileMetaData* f = s->physical_files[i];
      smeta.number = versions_->NewFileNumber();
      meta.number = versions_->NewFileNumber();
      vmeta.number = versions_->NewFileNumber();
      pending_outputs_.insert(meta.number);
    ...
    if (status.ok()) {
      ...
      for (size_t i = 0; i < tmp_vlogs.size(); i++) {
        c->edit()->DeleteVLog(c->level(), tmp_vlogs[i]->number);
      }
      c->edit()->DeleteSet(c->level(), tmp_set->number);
      const std::vector<FileMetaData*>& tmp_files = tmp_set->physical_files;
      for (size_t i = 0; i < tmp_files.size(); i++) {
        c->edit()->RemoveFile(c->level(), tmp_files[i]->number);
      }
    ...
    if (status.ok()) {
      SetMetaData smeta;
      smeta.number = versions_->NewFileNumber();
      smeta.smallest = c->GetStartKey();
      smeta.largest = c->GetLimitKey();
      c->edit()->AddSet(c->level() + 1, &smeta);
...
Status DBImpl::InstallCompactionResults(CompactionState* compact) {
  ...
  compact->compaction->AddInputFileDeletions(compact->compaction->edit());
  compact->compaction->CompactVLogs(compact->compaction->edit());
...
Status DBImpl::DoCompactionWork(CompactionState* compact, size_t input_idx) {
  ...
  while (input->Valid() && !shutting_down_.load(std::memory_order_acquire)) {
    ...
    if (!drop) {
      ...
      //     internal_comparator_.Compare(key, compact->largest.Encode()) > 0) {
      //   compact->largest.DecodeFrom(key);
      // }
    }
